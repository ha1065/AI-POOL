...
class GameState: #useless
    ...
    def create_white_ball(self):
        ...
        #print(self.balls)
        while check_if_ball_touches_balls(ball_pos, 0, self.balls): #moves bad ball?
            #print(ball_pos)
            #print("111111")
    ...
    def make_move(self):
        #print("making move")
        self.cue.angle = self.angle
        #print("angle:", str(self.cue.angle))
        self.cue.displacement = self.power
        #print("displacement:", str(self.cue.displacement))
        self.cue.ball_hit()
    ...
    def all_not_moving(self):
        return_value = True
        for ball in self.balls:
            #print(ball.ball.velocity)
            if np.count_nonzero(ball.ball.velocity) > 0:
                return_value = False
                break
        return return_value

    def reset_state(self):  #restarts game
    ...
    def generate_table(self):  #is this bad boy just useless?
        ...
        for hole_pos in all_hole_positions:
            self.holes.append(table_sprites.Hole(hole_pos[0][0], hole_pos[1][0], [hole_pos[0][1], hole_pos[1][1]]))
